...
posts_txts <- posts_tbl_processed %>%
  ...
  mutate(
    first_published_at = floor_date(first_published_at, "days"),
...
gamma_terms %>%
  ...
  labs(
    x = NULL,
    y = expression(gamma),
    title = "Tópicos ordenados por prevalência no conjunto de textos da New Order",
    subtitle = "Acompanhado das palavras que mais contribuem para cada um deles"
  )

ggsave(
  "plots/09-topics_by_prevalence.png",
  width = 29,
  height = 21,
  units = "cm"
)
...
ggsave(
  "plots/10-word_probability_by_topic 24 topics.png",
  width = 29,
  height = 21,
  units = "cm"
)
...
id_list <-
  findThoughts(model = topic_model,
               texts = as.character(meta$id),
               n = 10)
...
url_tbl <- read_rds("saved_data/url_tbl_20200901.rds")
...
gamma_terms %>%
  ...
  labs(
    ...
    y = expression(gamma),
    title = "Tópicos ordenados por prevalência no conjunto de textos da New Order",
    subtitle = "Acompanhado dos nomes gerados para cada um deles"
  )

ggsave(
  "plots/11-named_topics_by_prevalence.png",
  width = 25,
  height = 20,
  units = "cm"
)
...
network_plot <- ggnet2(
  ...
  geom_label_repel(
    aes(label = paste(
      topic_labels$topic, topic_labels$label, sep = " - "
    )),
    color = "white",
    fill = my_rainbow(24),
    fontface = "bold",
